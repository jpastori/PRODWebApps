<CFCOMPONENT displayname="SQL injection CFC" hint="This is the CFC for the SQL Injection problem">
	<CFFUNCTION name="SQL_injection" hint="sendEmails" returntype="string" access="remote">
		...
		<CFSET VARIABLES.insSql = "insert|delete|select|update|create|alter|drop|truncate|grant|revoke|declare|exec|backup|restore|sp_|xp_|execute|dbcc|deny|union|Cast|Char|Varchar|nChar|nVarchar">
		<CFSET VARIABLES.regEx="((null)+[[:space:]]*\(*'?[[:print:]]+'?([[:space:]]*[\+\-\/\*][[:space:]]*'?[[:print:]]+'?)*\)*[[:space:]]*(([=><!]{1,2}|(like))[[:space:]]*\(*'?[[:print:]]+'?([[:space:]]*[\+\-\/\*][[:space:]]*'?[[:print:]]+'?)*\)*)|((in)[[:space:]]*\(+[[:space:]]*'?[[:print:]]+'?(\,[[:space:]]*'?[[:print:]]+'?)*\)+)|((between)[[:space:]]*\(*[[:space:]]*'?[[:print:]]+'?(\,[[:space:]]*'?[[:print:]]+'?)*\)*(and)[[:space:]]+\(*[[:space:]]*'?[[:print:]]+'?(\,[[:space:]]*'?[[:print:]]+'?)*\)*)|((;)([^a-z>]*)(#VARIABLES.insSql#)([^a-z]+|$))|(union[^a-z]+(all|select))|(\/\*)|(--$))">

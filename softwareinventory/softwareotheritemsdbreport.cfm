...
<!--- Program: softwareotheritemsdbreport.cfm --->
<!--- By: JOHN R. PASTORI --->
<!--- Date Written: 07/13/2012 --->
<!--- Date in Production: 07/13/2012 --->
<!--- Module: IDT Software Inventory - Other Items Report --->
<!-- Last modified by John R. Pastori on 07/13/2012 using ColdFusion Studio. -->

<CFSET AUTHOR_NAME = "John R. Pastori">
<CFSET AUTHOR_EMAIL = "pastori@rohan.sdsu.edu">
<CFSET DOCUMENT_URI = "/#application.type#apps/softwareinventory/softwareotheritemsdbreport.cfm">
<CFSET CONTENT_UPDATED = "July 13, 2012">
<CFINCLUDE template = "../programsecuritycheck.cfm">

<HTML>
<HEAD>
	<TITLE>IDT Software Other Items Report</TITLE>
	<LINK rel="stylesheet" type="text/css" href="/webapps.css" />
<!-- Script starts here ---->
<SCRIPT language=JAVASCRIPT1.1>
	window.defaultStatus = "Welcome to IDT Software Inventory - Other Items Report";
...
<!--- 
*************************************************************************************************
* The following code is the Look Up Process for the IDT Software Inventory- Other Items Report. *
*************************************************************************************************
 --->
...
<CFIF NOT IsDefined('URL.PROCESS')>
	...
	<CFQUERY name="LookupSoftwareInventoryTitles" datasource="#application.type#SOFTWARE" blockfactor="100">
		SELECT	DISTINCT OI.SOFTWINVENTID, SI.SOFTWINVENTID,SI.TITLE, SI.VERSION, PP.PRODUCTPLATFORMNAME, SI.PURCHREQLINEID, PR.REQNUMBER,
				SI.TITLE || ' - ' || SI.VERSION || ' - ' || PP.PRODUCTPLATFORMNAME || ' - ' || PR.REQNUMBER AS LOOKUPKEY
		FROM		OTHERITEMS OI, SOFTWAREINVENTORY SI, PRODUCTPLATFORMS PP, PURCHASEMGR.PURCHREQLINES PRL,
				PURCHASEMGR.PURCHREQS PR
		WHERE	((OI.SOFTWINVENTID = 0 AND SI.SOFTWINVENTID = 0) OR
				(OI.SOFTWINVENTID = SI.SOFTWINVENTID AND
	...
	<TABLE width="100%" align="center" border="3">
		<TR align="center">
			<TD align="center">
				<H1>Select Data for IDT Software Inventory - Other Items Report Lookup</H1>
			</TD>
		</TR>
	</TABLE>
...
<CFFORM name="LOOKUP" onsubmit="return validateLookupFields();" action="/#application.type#apps/softwareinventory/softwareotheritemsdbreport.cfm?PROCESS=LOOKUP" method="POST">
		<TR>
		...
		<TR>
			...
			<TH align="left" valign="TOP"><LABEL for="REPORTCHOICE1">REPORT 1: &nbsp;&nbsp;All Software Other Items Titles</LABEL></TH>
			<TD align="LEFT" valign="TOP">
...
*****************************************************************************
 --->
	...
	<CFSET REPORTTITLE = ''>
	<CFSET OIRECORDCOUNT = 0>
	<CFSET PRINTRECORD = "YES">

	<CFIF #FORM.REPORTCHOICE# EQ 1>
		<CFSET REPORTTITLE = 'REPORT 1:&nbsp;&nbsp;&nbsp;&nbsp;All Software Other Items Titles'>

		<CFQUERY name="LookupSoftwareInventory" datasource="#application.type#SOFTWARE" blockfactor="100">
			SELECT	DISTINCT OI.SOFTWINVENTID AS OISWID, SI.SOFTWINVENTID, SI.CREATIONDATE, SI.TITLE, SI.VERSION, SI.CATEGORYID,
					PC.PRODCATID, PC.PRODCATNAME, SI.PRODPLATFORMID, PP.PRODUCTPLATFORMNAME, SI.FISCALYEARID, FY.FISCALYEARID,
					FY.FISCALYEAR_4DIGIT, SI.PURCHREQLINEID, PR.REQNUMBER, PR.PONUMBER, SI.PRODDESCRIPTION, SI.RECVDDATE,
					SI.PRODSTATUSID, SI.PHONESUPPORT, SI.WEBSUPPORT, SI.FAXSUPPORT, SI.SUPPORTCOMMENTS, SI.QTYORDERED, SI.LICENSETYPEID,
					SI.QTYLICENSED, SI.UPGRADESTATUSID, SI.TOSSSTATUSID, SI.CDKEY, SI.PRODUCTID, MANUFWARRVENDORID
			FROM		OTHERITEMS OI, SOFTWAREINVENTORY SI, PRODUCTCATEGORIES PC, PRODUCTPLATFORMS PP, LIBSHAREDDATAMGR.FISCALYEARS FY,
					PURCHASEMGR.PURCHREQLINES PRL, PURCHASEMGR.PURCHREQS PR
			WHERE	OI.SOFTWINVENTID = SI.SOFTWINVENTID AND
				<CFIF #FORM.TITLE# NEQ ''>
	...
	<CFIF #FORM.REPORTCHOICE# EQ 2>
		...
		<CFQUERY name="LookupSoftwareInventory" datasource="#application.type#SOFTWARE">
			SELECT	DISTINCT OI.SOFTWINVENTID AS OISWID, SI.SOFTWINVENTID, SI.CREATIONDATE, SI.TITLE, SI.VERSION,
					...
					SI.QTYORDERED, SI.LICENSETYPEID, SI.QTYLICENSED, SI.UPGRADESTATUSID, SI.TOSSSTATUSID, SI.CDKEY, SI.PRODUCTID,
					MANUFWARRVENDORID
			FROM		OTHERITEMS OI, SOFTWAREINVENTORY SI, PRODUCTCATEGORIES PC, PRODUCTPLATFORMS PP, LIBSHAREDDATAMGR.FISCALYEARS FY,
					PURCHASEMGR.PURCHREQLINES PRL, PURCHASEMGR.PURCHREQS PR
			WHERE	OI.SOFTWINVENTID = SI.SOFTWINVENTID AND
	...
	<CFIF #FORM.REPORTCHOICE# EQ 3>
		...
		<CFQUERY name="LookupSoftwareInventory" datasource="#application.type#SOFTWARE">
			...
			FROM		OTHERITEMS OI, SOFTWAREINVENTORY SI, PRODUCTCATEGORIES PC, PRODUCTPLATFORMS PP, LIBSHAREDDATAMGR.FISCALYEARS FY,
					STATUSES PS, LICENSETYPES LT, STATUSES UPGRS, STATUSES TOSS, PURCHASEMGR.VENDORS VEN, LIBSHAREDDATAMGR.CUSTOMERS CUST,
					PURCHASEMGR.PURCHREQLINES PRL, PURCHASEMGR.PURCHREQS PR
			WHERE	OI.SOFTWINVENTID = SI.SOFTWINVENTID AND
	...
	<TABLE width="100%" align="center" border="3">
		<TR align="center">
			<TD align="center">
				<H1>IDT Software Inventory - Other Items Report
				<H2>#REPORTTITLE#
			</H2></H1></TD>
		</TR>
	</TABLE>
	...
	<TABLE width="100%" align="center" border="0">
		...
		<TR>
<CFFORM action="/#application.type#apps/softwareinventory/softwareotheritemsdbreport.cfm" method="POST">
			<TD align="left">
...
<CFLOOP query="LookupSoftwareInventory">
	<CFIF #FORM.REPORTCHOICE# EQ 4>
		<CFQUERY name="LookupOtherItemsRecord" datasource="#application.type#SOFTWARE" blockfactor="100">
		...
		<CFIF #LookupOtherItemsRecord.RecordCount# GT 0>
			<CFSET PRINTRECORD = "YES">
		<CFELSE>
	...
	<CFIF #PRINTRECORD# EQ "YES">
		...
		<TR>
			...
			<TH align="CENTER" colspan="4">Other Items Title</TH>
			<TH align="center">Other Items Location</TH>
			<TH align="center">Other Items Quantity</TH>
		</TR>
	
		<CFQUERY name="LookupSoftwareOtherItemsDetail" datasource="#application.type#SOFTWARE">
			SELECT	SI.TITLE AS SOFTWTITLE, SI.VERSION, OI.PARTNUMBER, OI.TITLE AS OITITLE, OI.LOCATIONID, SL.STOREDLOCNAME, OI.OTHERITEMSQTY
			FROM		OTHERITEMS OI, SOFTWAREINVENTORY SI, STOREDLOCATIONS SL
			WHERE	OI.OTHERITEMSID > 0 AND
					OI.SOFTWINVENTID = <CFQUERYPARAM value="#LookupSoftwareInventory.SOFTWINVENTID#" cfsqltype="CF_SQL_NUMERIC"> AND
					OI.SOFTWINVENTID = SI.SOFTWINVENTID AND 
					OI.LOCATIONID = SL.STOREDLOCID
			ORDER BY	SI.TITLE, SI.VERSION, OI.TITLE
		</CFQUERY>
	
		<CFLOOP query="LookupSoftwareOtherItemsDetail">
			<CFSET OIRECORDCOUNT = #OIRECORDCOUNT# + 1>
		<TR>
			<TD align="CENTER" valign="TOP"><DIV>#LookupSoftwareOtherItemsDetail.PARTNUMBER#</DIV></TD>
			<TD align="LEFT" valign="TOP" colspan="4"><DIV>#UCase(LookupSoftwareOtherItemsDetail.OITITLE)#</DIV></TD>
			<TD align="CENTER" valign="TOP"><DIV>#LookupSoftwareOtherItemsDetail.STOREDLOCNAME#</DIV></TD>
			<TD align="CENTER" valign="TOP"><DIV>#LookupSoftwareOtherItemsDetail.OTHERITEMSQTY#</DIV></TD>
		</TR>
		</CFLOOP>
		<TR>
		...
		<TR>
			<TH align="CENTER" colspan="7">
				<H2>#OIRECORDCOUNT# Software Inventory Other Items records were selected.
			</H2></TH>
		</TR>
		<TR>
<CFFORM action="/#application.type#apps/softwareinventory/softwareotheritemsdbreport.cfm" method="POST">
